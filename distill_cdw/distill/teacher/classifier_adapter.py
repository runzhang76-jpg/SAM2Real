...
from distill.teacher.dinov3_knn_classifier import DINOv3KNNClassifier
from distill.utils.logging import get_logger
from segment_cdw.classier_network.CNNs import seresnext50_32x4d
...
class _BaseClassifierBackend:
    def predict(self, patches: List[np.ndarray]) -> List[Dict[str, Any]]:
        raise NotImplementedError


class _ModelClassifierBackend(_BaseClassifierBackend):
    ...
    def _build_model(self) -> None:
        ckpt_path = str(self.cfg.get("checkpoint", "")).strip()
        if not ckpt_path:
    ...
    def predict(self, patches: List[np.ndarray]) -> List[Dict[str, Any]]:
        if self.model is None or len(patches) == 0:
            return []
        patch_tensors = [self.patch_transform(p) for p in patches]
        ...
        with torch.no_grad():
            for i in range(0, len(patch_tensors), self.batch_size):
                ...
                outputs.extend(
                    [
                        {"category_id": int(c) + 1, "category_score": float(p)}
                        for c, p in zip(cls.detach().cpu().tolist(), conf.detach().cpu().tolist())
                    ]
                )
        return outputs


class _DINOv3KNNBackend(_BaseClassifierBackend):
    def __init__(self, cfg: Dict[str, Any], logger: Any) -> None:
        self.classifier = DINOv3KNNClassifier(cfg=cfg, logger=logger)

    def predict(self, patches: List[np.ndarray]) -> List[Dict[str, Any]]:
        return self.classifier.predict_patches(patches)


class _DINOv3PrototypeBackend(_BaseClassifierBackend):
    def __init__(self, cfg: Dict[str, Any], logger: Any) -> None:
        self.classifier = DinoV3PrototypeClassifier(cfg=cfg, logger=logger)
...
class ClassifierAdapter:
    ...
    def __init__(self, cfg: Dict[str, Any]) -> None:
        ...
        self.backend: Optional[_BaseClassifierBackend] = self._build_backend()
    ...
    def _build_backend(self) -> Optional[_BaseClassifierBackend]:
        if self.type == "none":
        ...
        if self.type == "model":
            ...
            return _ModelClassifierBackend(model_cfg, logger=self.logger)
        if self.type == "dinov3_knn":
            dino_cfg = dict(self.cfg)
            dino_cfg.update(self.cfg.get("dinov3_knn", {}))
            return _DINOv3KNNBackend(dino_cfg, logger=self.logger)
        if self.type in {"prototype_similarity", "dinov3_prototype"}:
            dino_cfg = dict(self.cfg)
            dino_cfg.update(self.cfg.get("dinov3_prototype", {}))
            return _DINOv3PrototypeBackend(dino_cfg, logger=self.logger)
    ...
    def classify(self, instances: List[InstancePrediction], image_np: Optional[np.ndarray] = None) -> List[InstancePrediction]:
        ...
        try:
            cls_outputs = self.backend.predict(patches)
