#!/usr/bin/env python
"""Run existing DINOv3 k-NN classifier on GT object crops for debugging."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Debug DINOv3 k-NN classification with GT object boxes")
    parser.add_argument("--config", default="distill_cdw/configs/distill_default.yaml")
    parser.add_argument("--gt-json", default="../data/cdw_classify/subtest/instances_Subtest_0based.json")
    ...
    parser.add_argument("--out-json", default="outputs/gt_knn_predictions.json")
...
def main() -> None:
    ...
    dino_cfg = dict(cls_cfg)
    dino_cfg.update(cls_cfg.get("dinov3_knn", {}))
    ...
    if str(cls_cfg.get("type", "")).lower() != "dinov3_knn":
        logger.warning("teacher.classifier.type is not dinov3_knn; script still uses dinov3_knn config fields")

    knn = DINOv3KNNClassifier(cfg=dino_cfg, logger=logger)
    ...
    for ann in anns:
        ...
        if image_id not in image_cache:
            ...
            if not image_path.exists():
                ...
                skipped += 1
                continue
            image_cache[image_id] = np.asarray(Image.open(image_path).convert("RGB"))
    ...
    preds = knn.predict_patches(patches)
    ...
    logger.info(
        "GT->kNN done: total=%d skipped=%d correct=%d acc=%.4f use_mask=%s margin_ratio=%.3f",
        total,
        skipped,
        correct,
        acc,
