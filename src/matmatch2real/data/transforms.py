...
from matmatch2real.student.distill_soft.io import flip_teacher_soft, resize_teacher_soft
        ...
        for inst in instances:
            ...
            if inst.mask is not None:
                ...
                if isinstance(mask, torch.Tensor):
                    ...
                    inst.mask = mask[:, ::-1].copy()

        meta = sample.get("meta", {})
        meta["flipped"] = True
        sample["meta"] = meta
        teacher_soft = sample.get("teacher_soft")
        if isinstance(teacher_soft, dict):
            sample["teacher_soft"] = flip_teacher_soft(teacher_soft)
        return sample
...
class Resize:
    ...
    def __call__(self, sample: Dict[str, Any]) -> Dict[str, Any]:
        ...
        teacher_soft = sample.get("teacher_soft")
        if isinstance(teacher_soft, dict):
            sample["teacher_soft"] = resize_teacher_soft(teacher_soft, (new_h, new_w))
        return sample
