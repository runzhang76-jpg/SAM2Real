...
class SwanLabRemote:
    ...
    def _try_sdk_sync(
        ...
        def _call(obj: Any, method: str) -> bool:
            ...
            try:
                ...
                self.logger.warning("SwanLab SDK (%s.%s): %s", obj, method, exc)
                return False
        ...
        if dataset_cls is not None:
            try:
                ...
                self.logger.warning("SwanLab SDK Dataset : %s", exc)
    ...
    def sync_up(
        ...
        if self._try_sdk_sync("up", local_path, remote_uri, includes, excludes):
            self.logger.info("[swanlab] SDK : %s -> %s", local_path, remote_uri)
            return
        if self._cli_path:
            self._run_cli("up", local_path, remote_uri, includes, excludes)
            return
        self.logger.warning("SwanLab CLI ")
    ...
    def sync_down(self, remote_uri: str, local_path: Path, overwrite: bool = False) -> None:
        ...
        if self._try_sdk_sync("down", local_path, remote_uri, None, None, overwrite=overwrite):
            self.logger.info("[swanlab] SDK : %s -> %s", remote_uri, local_path)
            return
        if self._cli_path:
