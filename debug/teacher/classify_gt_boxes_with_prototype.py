...
def main() -> None:
    ...
    proto_cfg = dict(cls_cfg.get("dinov3", {}))
    proto_cfg["mode"] = "prototype"
    proto_cfg.update(proto_cfg.get("prototype", {}))
    ...
    if str(cls_cfg.get("type", "")).lower() != "dinov3":
        logger.warning("teacher.classifier.type is not dinov3; script still forces dinov3.prototype config fields")

    classifier = DinoV3Classifier(cfg=proto_cfg, logger=logger)
