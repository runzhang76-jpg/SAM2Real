#!/usr/bin/env python
"""Visualize GT boxes vs Watershed prompt boxes on one image."""
...
from matmatch2real.teacher.prompt_generators import WatershedBoxPromptGenerator
...
def _greedy_match_stats(gt_boxes: List[List[float]], ws_boxes: List[List[float]], iou_thr: float) -> Dict[str, float]:
    used_gt = set()
    matched = 0
    for wb in ws_boxes:
        best_iou = 0.0
    ...
    gt_n = len(gt_boxes)
    ws_n = len(ws_boxes)
    precision = matched / ws_n if ws_n > 0 else 0.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out", default="outputs/watershed_vs_gt_boxes.png", help="Output visualization path")
    parser.add_argument("--config", default="", help="Optional config YAML to read watershed params")
    parser.add_argument("--blur-kernel", type=int, default=5)
    parser.add_argument("--threshold-mode", default="otsu", help="otsu or fixed")
    parser.add_argument("--binary-inv", type=lambda x: str(x).lower() in {"1", "true", "yes", "y"}, default=True)
    ...
    parser.add_argument("--dist-thresh-ratio", type=float, default=0.4)
    parser.add_argument("--min-box-w", type=int, default=0)
    parser.add_argument("--min-box-h", type=int, default=0)
...
def main() -> None:
    ...
    if args.config:
        ...
        ws_cfg.update(cfg.get("teacher", {}).get("prompt_generator", {}).get("watershed_boxes", {}))

    generator = WatershedBoxPromptGenerator(ws_cfg)
    ws_boxes_np = generator.generate_boxes(image_np, image_meta=img_info)
    ws_boxes = ws_boxes_np.tolist()

    stats = _greedy_match_stats(gt_boxes, ws_boxes, iou_thr=args.iou_thr)
    print(
        f"image_id={image_id}, file={file_name}, "
        f"gt={int(stats['gt_count'])}, watershed={int(stats['ws_count'])}, "
    ...
    ws_view = _draw_boxes(image_np, ws_boxes, color=(255, 200, 60), title="Watershed boxes")
    ...
    for b in ws_boxes:
        draw.rectangle(tuple(b), outline=(255, 200, 60), width=2)
    ...
    merged = _stack_h([gt_view, ws_view, overlay])
