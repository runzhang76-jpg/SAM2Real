...
from sam2real.teacher.dinov3_classifier import DinoV3Classifier
...
def main() -> None:
    ...
    if str(cls_cfg.get("type", "")).lower() != "dinov3":
        logger.warning("teacher.classifier.type is not dinov3; script still forces dinov3.knn config fields")

    knn = DinoV3Classifier(cfg=dino_cfg, logger=logger)
