#!/usr/bin/env python
"""Visualize GT boxes vs Canny prompt boxes on one image."""
...
from matmatch2real.teacher.prompt_generators import CannyBoxPromptGenerator
...
def _greedy_match_stats(gt_boxes: List[List[float]], canny_boxes: List[List[float]], iou_thr: float) -> Dict[str, float]:
    used_gt = set()
    matched = 0
    for cb in canny_boxes:
        best_iou = 0.0
        best_j = -1
        for j, gb in enumerate(gt_boxes):
            if j in used_gt:
                continue
            iou = _box_iou_xyxy(cb, gb)
            if iou > best_iou:
                best_iou = iou
                best_j = j
    ...
    gt_n = len(gt_boxes)
    canny_n = len(canny_boxes)
    precision = matched / canny_n if canny_n > 0 else 0.0
...
def _draw_boxes(image: np.ndarray, boxes: List[List[float]], color: Tuple[int, int, int], title: str) -> Image.Image:
    ...
    draw.rectangle((8, 8, 8 + 260, 38), fill=(0, 0, 0))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="", help="Optional config YAML to read canny params")
...
def main() -> None:
    ...
    if args.config:
        ...
        canny_cfg.update(cfg.get("teacher", {}).get("prompt_generator", {}).get("canny_boxes", {}))

    generator = CannyBoxPromptGenerator(canny_cfg)
    canny_boxes_np = generator.generate_boxes(image_np, image_meta=img_info)
    canny_boxes = canny_boxes_np.tolist()

    stats = _greedy_match_stats(gt_boxes, canny_boxes, iou_thr=args.iou_thr)
    print(
        f"image_id={image_id}, file={file_name}, "
        f"gt={int(stats['gt_count'])}, canny={int(stats['canny_count'])}, "
    ...
    canny_view = _draw_boxes(image_np, canny_boxes, color=(255, 80, 80), title="Canny boxes")

    overlay = Image.fromarray(image_np.copy())
    draw = ImageDraw.Draw(overlay)
    for b in gt_boxes:
        draw.rectangle(tuple(b), outline=(80, 255, 80), width=3)
    for b in canny_boxes:
        draw.rectangle(tuple(b), outline=(255, 80, 80), width=2)
    draw.rectangle((8, 8, 8 + 480, 44), fill=(0, 0, 0))
    draw.text(
        (12, 12),
    ...
    merged = _stack_h([gt_view, canny_view, overlay])
